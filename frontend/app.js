...
async function handleAuthSubmit(event) {
    ...
    if (currentAuthType === 'staff' && currentAuthMode === 'register') {
        showToast("Registration Denied: Service Agent registration must be performed by an Administrator.");
        return;
    }
    let role = 'Customer';
...
async function loadCustomerTicket() {
    ...
    try {
        ...
        const isCompletedOrSkipped = freshTicket.status === 'COMPLETED' || freshTicket.status === 'SKIPPED';
        const cancelBtn = document.getElementById("cancelTicketBtn");
        
        if (isCompletedOrSkipped) {
            if (cancelBtn) cancelBtn.classList.add("hidden");
            if (freshTicket.status === 'COMPLETED') {
                statusBadge.style.backgroundColor = "rgba(100, 116, 139, 0.12)";
                statusBadge.style.color = "#64748b";
                if (cachedTicket.status !== 'COMPLETED') {
                    showToast("Your ticket has been marked Completed!");
                }
            } else if (freshTicket.status === 'SKIPPED') {
                statusBadge.style.backgroundColor = "rgba(245, 158, 11, 0.12)";
                statusBadge.style.color = "#f59e0b";
                if (cachedTicket.status !== 'SKIPPED') {
                    showToast("Your ticket was skipped (no-show).");
                }
            }
        } else {
            if (cancelBtn) cancelBtn.classList.remove("hidden");
            statusBadge.removeAttribute("style"); // reset inline styles
        ...
        if (isCompletedOrSkipped) {
            document.getElementById("ticketWaitCount").innerText = "0";
            document.getElementById("ticketWaitTime").innerText = freshTicket.status === 'COMPLETED' ? "Completed" : "Skipped";
        } else {
            // Estimate wait times
...
async function refreshStaffDeskData(queueId) {
    ...
    try {
        ...
        const agentSelect = document.getElementById("staffUserSelect");
        const currentAgentId = agentSelect ? parseInt(agentSelect.value) : null;

        // Filter tokens to only show WAITING tokens and tokens served by the current agent
        const filteredTokens = tokens.filter(t => {
            if (t.status === 'WAITING') return true;
            return t.user && t.user.userId === currentAgentId;
        });

        // Filter and update backlog metrics
        const waitingTokens = filteredTokens.filter(t => t.status === 'WAITING').sort((a, b) => a.tokenNumber - b.tokenNumber);
...
function renderServiceHistoryTable() {
    ...
    if (filteredHistory.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" class="text-center text-muted">No completed service records found for the selected criteria.</td></tr>`;
        return;
    }
    ...
    sortedHistory.forEach(item => {
        ...
        const facilityName = (item.token && item.token.queue) ? item.token.queue.queueName : "Unknown Facility";
        ...
        if (dateLabel !== lastDateLabel) {
            ...
            tableBody.innerHTML += `
                <tr class="date-group-header">
                    <td colspan="7" style="background-color: rgba(255, 255, 255, 0.03); font-weight: 700; color: var(--primary-hover); font-size: 13px; letter-spacing: 0.5px; border-bottom: 1px solid var(--border-color); padding: 12px 20px;">
...
// Format raw LocalDateTime ISO string to a human-readable layout
function formatDateTime(str) {
    if (!str) return "N/A";
    const date = parseLocalDateTime(str);
...
async function openMyTokensModal() {
    ...
    tableBody.innerHTML = `<tr><td colspan="6" class="text-center text-muted">Loading your tokens...</td></tr>`;
    ...
    try {
        ...
        if (tokens.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="6" class="text-center text-muted">No generated tokens found for your account.</td></tr>`;
            return;
        }
            ...
            const actionButtons = `
                <div style="display:flex; gap: 8px;">
                    ${(t.status === 'WAITING' || t.status === 'SERVING') ? `
                        <button class="btn btn-secondary" style="padding: 4px 8px; font-size:12px;" onclick="event.stopPropagation(); trackToken(${t.tokenId})">Track</button>
                        <button class="btn btn-danger" style="padding: 4px 8px; font-size:12px; background-color: var(--danger);" onclick="event.stopPropagation(); cancelTokenFromHistory(${t.tokenId})">Cancel</button>
                    ` : `-`}
                </div>
            `;

            tableBody.innerHTML += `
                <tr onclick="trackToken(${t.tokenId})" style="cursor: pointer;" title="Click to view ticket">
                    <td style="font-weight: 700;">${formatTokenNumber(t.tokenNumber)}</td>
                    <td>${queueName}</td>
                    <td>${t.customerName}</td>
